...
parser.add_argument("-p","--project-file",required=True,help="path to JSON projects file")
parser.add_argument("-v","--verbose",default=False,action="store_true",help="Verbose on/off")
parser.add_argument("-c","--commit",default=None,help="only test one commit")
parser.add_argument("-r","--repo",default=None,help="repo to test, default is first key in project file")
...
while True:
    for r in repos:
        p = projects[r]
        kargs = {"target":cibot.check_project,"args":(p,),"kwargs":{"verbose":args.verbose}}
